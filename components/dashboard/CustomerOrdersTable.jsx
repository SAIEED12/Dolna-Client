...
export function CustomerOrdersTable({ orders }) {
  ...
  const rows = useMemo(() => {
    ...
    return list.filter((order) => order.orderStatus === statusFilter);
  }, [orders, statusFilter]);

  return (
    <>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <label htmlFor="my-orders-status" className="sr-only">
          Filter by status
        </label>
        <select
          id="my-orders-status"
      ...
      <Table>
        <Table.ScrollContainer>
          <Table.Content aria-label="My orders" className="min-w-190">
            <Table.Header>
      ...
      {rows.length === 0 ? (
        <p className="mt-4 rounded-2xl border border-dashed border-line bg-white px-6 py-10 text-center text-sm text-smoke">
          No orders found. Try a different status filter.
        </p>
      ) : null}
